...
def soln_density(wts_pc,wtn_pc,T):
    """  
...
def lorentz_lorenz(m0,T0,T,wtp):
    ...
    Input:
        ...
        wtp: the sulfate weight fraction (0-1)
...
def sulfate_wtp(press,T,H2Ovmr):
    """
     This routine returns the equilibrium composition
        of an aerosol composed of water and sulfuric acid
        in weight percent of H2SO4 as a function of temp
        pressure and H2O mixing ratio.
        Uses an equation published in Hanson et al., 1994 JGR,
        which is a fit to the data of Steel and Hamill 1981     
    Input:
       temp_in.......temp (K)
       qh2o_in.......water vapor mixing ratio (ppmv)
       pres_in.......pressure (mb)
    Returns
       sulfatef......the sulfuric acid composition (wt %)
       """
...
def refractive_index(wvl,T,P,H2Ovmr):
    """
    Returns the real and iamginary refractive indicies for a specified wavelenght
    based on:
        wvl - Wavelenght in nm
        T - Temperature in Kelvin
        P - hPa/mb
        H20Ovmr - water vapor volume mixing ratio in ppm
    ...
    m, k = pandw(wvl,wtp)
    m_corrected, A = lorentz_lorenz(m,300,T,wtp)
    #k_corrected = lorentz_lorenz(k,300,T,wtp)
    
    return m_corrected, wtp
